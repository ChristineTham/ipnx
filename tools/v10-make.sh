...
#
# See tools/v10-make.exp for what is being asserted.
set -uo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
source "$ROOT/tools/v8clone.sh"

TPORT="${TPORT:-9230}"; OPORT="${OPORT:-9231}"
MPORT="${MPORT:-9232}"; SPORT="${SPORT:-9233}"
SCRATCH="$ROOT/work/v10mk"
...
IMG=$(v8_clone "${1:-rp07new}" mk) || exit 1
...
expect "$ROOT/tools/v10-make.exp" "$IMG" "$TPORT" "$OPORT" "$MPORT" "$SPORT" 2>&1 \
