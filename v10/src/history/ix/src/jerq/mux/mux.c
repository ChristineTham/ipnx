...
scan()
{
	...
	if(rdfd.fds_bits[0]==0) {
		...
		if(bit&rdfd.fds_bits[0]){
			...
			ifdeftracing(if(n==0) trace("0 byte read", (char*)0));
			if(fd==0){
				static neof;
				if(n==0) {
					if (++neof > 10)
	...
	register char *s;
{
	...
	ifdeftracing(trace("\nmux: %s\n", s); trace((char*)0, (char*)0); fflush(tracefd));
	if(booted){
		for(i=0; i<NSELFD; i++)
			if(layer[i].busy)
				(void)close(i);
	...
	int n;
{
	...
	case M_IOCTL:
		...
		case TIOCGETC:
			...
			break;
		case TIOCFLUSH:
			/* should propagate flush to terminal */
		case TIOCSDEV:
			layer[fd].ttydev=*(struct ttydevb *)(bp+MSGHLEN+sizeof(int));
			ttyset(fd, &layer[fd].ttychars);
			size=0;
			break;
		case TIOCGDEV:
			size=sizeof(devsave)+sizeof(int);
			*(struct ttydevb *)(bp+MSGHLEN+sizeof(int))=layer[fd].ttydev;
			break;
		case JMUX:
			size=0;
			break;
	...
	struct ttychars *tp;
{
	...
	m.ttychars=*tp;
	m.ttychars.flags1 = layer[fd].ttydev.flags; /* use delay byte for flags */
	i=psend_hold(0, (char *)&m, sizeof m, fd);
	trace("psend ttyioctl %d\n", i);
}
sendioctl(fd, cmd)
{
	char ioctlvec[2];
	...
	register int cc;
{
	...
	while(cc--){
		...
		case C_SENDNCHARS:	/* send cc characters */
			wrmesgb(i, s, cc);
			return 0;
		case C_SENDNCHARD:	/* send cc characters with delimeter */
			wrmesgb(i, s, cc);
			delim(i);
			return 0;
	...
	char *argv[];
{
	if(strcmp(argv[0], "cd")==0){
		...
		while(*where){
			strncpy(cdbuf, where, CDSIZE);
			ioctl(0, JCHDIR, cdbuf);
			where+=strlen(cdbuf);
		}
