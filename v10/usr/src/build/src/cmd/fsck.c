...
#endif
#ifdef mips
#define MAXDATA	(0x10000000+(MEMSIZE)400*1024)	/* hack */
#endif
#ifndef MAXDATA
#define MAXDATA ((MEMSIZE)200*1024)
#endif
...
long	*bm_buf;		/* ptr to bit map buffer area */
...
int	zcleared;		/* num of empty, linkless files cleared */
...
daddr_t m_blks;			/* number of bitmap blocks in the file system */
...
char	*argv[];
{
	...
	minsize = 2*sizeof(BUFAREA)+BSIZE(BIG);
	...
	char	*name;
{
  ...
  retry:
	...
	big = BIG;		/* 1k blocks by default */
	if (S_ISBLK(stat_block.st_mode)){
 ...
 * It printfs a message and then dies.
...
char *dev;
{
	...
	rawflg = 0;
	big = BIG;	/* does this undo the work in blockcheck? */
	if(S_ISBLK(statarea.st_mode)) {
		if(BITFS(statarea.st_rdev))
			big = BIG;
	}
	else if(S_ISCHR(statarea.st_mode))
		rawflg++;
	else {
	...
	m_blks = 0;
	/*
         * if this is a big bit mapped file system,
	 *	back off fmax by m_blks the number of bitmap blocks.
	 */
	if ((big == BIG) && superblk.U.N.S_flag) {
		m_blks = (superblk.s_fsize+superblk.U.N.S_bsize-1)/superblk.U.N.S_bsize;
        	fmax -= m_blks;
	}
	if(fmin >= fmax || 
	...
	bm_buf = (long *)membase;
	memsize = memsize-BSIZE(BIG);
	membase = membase+BSIZE(BIG);
	...
	if((off_t)msize < totsz) {
		...
		if(stat(scrfile,&statarea) < 0 || S_ISREG(statarea.st_mode))
			rmscr++;
...
freechk()
{
	...
	if(big == BIG) {
		if (m_blks) {
			blkno = bitno = superblk.s_isize;
                        bm_blk = fmax;
                        memset((char *)bm_buf, 0, BSIZE(BIG));
                        while(blkno < fmax) {
				if(!getfmap(blkno++)) {
                                        BITFREE(bm_buf, bitno);
                                        n_free++;
                                }
                                if(++bitno == BITSPERBLK) {
                                        bwrite(&dfile, (char *)bm_buf, bm_blk, BSIZE(BIG));
                                        bm_blk++;
			                memset((char *)bm_buf, 0, BSIZE(BIG));
                                        bitno = 0;
                                }
			...
			memset((char *)superblk.s_bfree, 0, BITMAP);
			for(blkno = superblk.s_isize; blkno < fmax; blkno++) {
                                if(!getfmap(blkno)) {
                                        BITFREE(superblk.s_bfree, blkno-superblk.s_isize);
                                        n_free++;
                                }
			}
		}
