...
# lands in /usr.  The five partitions used here tile the drive exactly, and
# they tile an RA81 too -- f simply ends sooner:
#
#	a       10,240 @       0        5 MB    root
#	b       20,480 @  10,240       10 MB    swap, and the dump area
#	c      249,848 @  30,720      122 MB    swap    (alice.m:5's size)
#	d      249,848 @ 280,568      122 MB    swap    (alice.m:6's size)
#	e      249,848 @ 530,416      122 MB    spare
#	f    3,140,226 @ 780,264     1533 MB    /usr
#
# g is not a fifth partition: ra.c:56 puts it at 30,720 for 749,544 blocks,
...
# THREE SWAP AREAS, BECAUSE ONE OF 10 MB IS NOT A MACHINE THAT BUILDS.
# ENOMEM on this kernel is a SWAP shortage and nothing else: of the three
# sites that set it, vmproc.c:294 is maxdsize (819200 clicks = 419 MB) and
# sys1.c:809 is per-process LIM_DATA, while vmdrum.c:49 is swpexpand's
# vsexpand failing to find room -- and that is the one a build hits.  With
# ra0b alone the whole machine shared 20,480 blocks, so cmd/spell's pcode
# (pcode.c:36-37, `Dict words[200000]' and `char space[500000]', ~2.1 MB of
# bss) answered `sh: ./pcode: Not enough memory' and backup.old's second mk
# answered `mk fork: Not enough memory'.
#
# THE TAPE'S OWN 780 SAYS HOW MUCH.  astro/alice.m:5-6 -- the config this
# one derives from -- is `swap ra 042 249848' and `swap ra 043 249848':
# two areas of 122 MB on partitions c and d, 244 MB in total, 24 times what
# we had.  bowell.m declares four and readconf.c:199 is `#define NSWAP 10',
# so more than one is the ordinary arrangement rather than an extension.
#
# AND THE ROOM WAS ALREADY THERE, IDLE.  io/ra.c:49-57 fixes the partitions:
# c is 249,848 @ 30,720, d is 249,848 @ 280,568, and f -- /usr -- does not
...
# probes with a longword.  Until then the way into this machine is the share:
# 9P on V10 (/etc/9pfs over netb), netfs on V8.
tm03 0	mb 1	drive 0
te16 0	ctl 0	unit 0
