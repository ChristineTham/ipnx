...
Bitmap stardwg={starbits, 1, 0, 0, 16, 16};
short p0bits[]={
#include "player0.icon"
};
Bitmap p0dwg={p0bits, 3, 0, 0, 44, 44};
short p1bits[]={
#include "player1.icon"
};
Bitmap p1dwg={p1bits, 3, 0, 0, 44, 44};
short misbits[]={
#include "missile.icon"
};
Bitmap misdwg={misbits, 2, 0, 0, 32, 32};
short boombits[]={
#include "boom.icon"
};
Bitmap boomdwg={boombits, 4, 0, 0, 64, 64};
struct obj{
	int x, y;
...
kbdplayer(c){
	...
	case 0xD1: jerk(P0);	break;
	case 0xC2: right(P0);	break;
	case 0xC1: kill(P0);	break;
	case 0xB1: hyper(P0);	break;
	case 'a':  left(P1);	break;
	case 'w':  jerk(P1);	break;
	case 'd':  right(P1);	break;
	case 's':  kill(P1);	break;
	case 'z':
