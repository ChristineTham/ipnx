...
start:
		for (i = 0; i < addrblock.c_count; i += 2) {
			if (addrblock.c_addr[i])
				...
				clearbuf(buf, 0);
			if (size > bSIZE && addrblock.c_addr[i+1])
				readtape(buf, 1);
			else
				clearbuf(buf, 1);
			if (addrblock.c_addr[i] || size > bSIZE && addrblock.c_addr[i + 1])
...
loop:
	...
	copy(&tbf[(bct++*bSIZE)], b + part * bSIZE, bSIZE);
}
