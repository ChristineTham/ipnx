/* RECONSTRUCTED HEAD.  The copy on v10src is damaged: its first 4096 bytes are
   not this file at all -- bytes 0-2690 are the tail of GNU CC's Makefile (the
   genflags and cccp rules, install and maketest), and bytes 2691-4095 are 1405
   NUL bytes.  file(1) calls the result "data".  cpp reads the Makefile comment
   lines as directives:

       local-alloc.c: 42: undefined control

   The damage is upstream, not this project's: v10src.tar.bz2 downloaded fresh
   from TUHS has md5 a07490fb33a6d83d4578dadb40a6aef4, identical to ours, and
   its local-alloc.c has sha256 8d3253a22b01f0dc8429bcab9d43648e56d6c5d385b6e-
   347f3dca0ede1882db7, identical to the extracted copy, with the same Makefile
   text and the same NUL run.

   Everything from byte 4096 is intact and is kept verbatim.  Of the GNU CC 1.x
   releases, 1.21 matches that surviving text far better than any other -- a
   difflib ratio of 0.897 against 0.66 for the next best, 1.29 and 1.30 -- so
   the head below is 1.21's, up to the comment the tape resumes inside.

   One declaration is added.  The surviving text uses qty_min_class eight times
   and declares it nowhere, because its declaration sat in the lost block; 1.21
   calls that variable qty_reg_class, and the rename to qty_min_class arrives in
   1.25.  The comment and declaration below are 1.25's, verbatim.  Nothing else
   is added: next_qty, qty_phys_reg, qty_phys_sugg and qty_birth come from 1.21's
   head and the surviving text does not redeclare them.  */
