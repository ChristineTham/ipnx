	...
	struct symtab **Pptr, **Qptr;
{
	 ...
	 */
	if (p->s_tag >= IGNOREBOUND && q->s_tag < IGNOREBOUND)	/*p points to a jxxx*/
		return(-1);		
	if (q->s_tag >= IGNOREBOUND && p->s_tag < IGNOREBOUND)
		return(1);
	/*
...
/*		reloc.r_addsyl = isaddr!=0;(addsyl is unreferenced)*/
