#include <stdio.h>	/* ipnx stdio -- see PATCHES.md */
#
# include "mcons.c"
char	ib1[518];
FILE	*ibuf;
...
/*	printf("collisions = %d\nsymbols = %d\n",ncol,nsym); /*INSTR*/
	i = 0100200;
	error = write(ofile,&i,sizeof(i));
	if(error < 0)err("write",argv[2]);
	i = sizeof(*xtab.hptr)*xtab.hsiz;
	error = write(ofile,&i,sizeof(i));
	if(error < 0)err("write",argv[2]);
	error = write(ofile,&xtab.ssiz,sizeof(xtab.ssiz));
	if(error < 0)err("write",argv[2]);
