...
int bct = NTREC+1;
char tbf[NTREC*BSIZE(0)];

struct	cache {
	daddr_t	c_bno;
	int	c_time;
	char	c_block[BSIZE(0)];
} cache[NCACHE];
int	curcache;
...
start:
		for (i = 0; i < addrblock.c_count; i++) {
			if (addrblock.c_addr[i]) {
				readtape(buf);
				(*f1)(buf, size > BSIZE(0) ? (long) BSIZE(0) : size);
			}
			else {
				clearbuf(buf);
				(*f2)(buf, size > BSIZE(0) ? (long) BSIZE(0) : size);
			}
			if ((size -= BSIZE(0)) <= 0) {
eloop:
				while (gethead(&spcl) == 0)
...
char *b;
{
	...
	if (bct >= NTREC) {
		for (i = 0; i < NTREC; i++)
			((struct spcl *)&tbf[i*BSIZE(0)])->c_magic = 0;
		bct = 0;
		if ((i = read(mt, tbf, NTREC*BSIZE(0))) < 0) {
...
loop:
	...
	copy(&tbf[(bct++*BSIZE(0))], b, BSIZE(0));
}
...
char	*b;
{
	...
	for (i = 0; i < NCACHE; i++) {
		if (cache[i].c_bno == bno) {
			copy(b, cache[i].c_block, BSIZE(0));
			cache[i].c_time = 0;
			break;
		}
		else
	...
	lseek(fi, bno*BSIZE(0), 0);
	if(write(fi, b, BSIZE(0)) != BSIZE(0)) {
#ifdef STANDALONE
...
char *buf;
{
	...
	lseek(fi, bno*BSIZE(0), 0);
	if (read(fi, cache[j].c_block, BSIZE(0)) != BSIZE(0)) {
#ifdef STANDALONE
		printf("read error %D\n", bno);
#else
...
int	f1, f2;
{
	...
	union {
		char	data[BSIZE(0)];
		daddr_t	indir[NINDIR(0)];
	} ibuf;

	dread(bn, ibuf.data, BSIZE(0));
	for(i=NINDIR(0)-1;i>=0;i--) {
		nb = ibuf.indir[i];
		if(nb) {
			if(f1)
				tloop(nb, f2, 0);
...
char *b;
{
	...
	for (dp = (struct direct *) b, i = 0; i < BSIZE(0); dp++, i += sizeof(*dp)) {
		if (dp->d_ino == 0)
			continue;
		putent((char *) dp);
	}
...
struct	dinode *b;
{
	...
	bno = (ino - 1)/INOPB(0);
	bno += 2;
	dread(bno, buf, BSIZE(0));
	copy(&buf[((inum-1)%INOPB(0))*sizeof(struct dinode)], (char *) b, sizeof(struct dinode));
}
...
struct	dinode *b;
{
	...
	bno = ((ino - 1)/INOPB(0)) + 2;
	dread(bno, buf, BSIZE(0));
	copy((char *) b, &buf[((inum-1)%INOPB(0))*sizeof(struct dinode)], sizeof(struct dinode));
	dwrite(bno, buf);
}
