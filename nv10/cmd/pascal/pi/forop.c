...
static char sccsid[] = "@(#)forop.c 1.9 6/1/81";
...
#	endif PC
	...
	putline();
	lhs = (int *)( (int *) arg[2] )[2];
	init = (int *)( (int *) arg[2] )[3];
	term = (int *)arg[3];
	stat = (int *)arg[4];
	if (lhs == NIL) {
nogood:
	    if (forvar != NIL) {
		forvar->u.vals.value[ NL_FORV ] = FORVAR;
	    }
	...
	s_forv = forvar ->u.vals.value[ NL_FORV ];
	if ( lhs[3] != NIL ) {
	...
	if ( opt('s') &&
	    ( ( bn != cbn ) ||
#ifdef OBJ
		( whereis( bn , forvar->u.vals.value[NL_OFFS] , 0 ) == PARAMVAR )
#endif OBJ
#ifdef PC
		( whereis( bn , forvar->u.vals.value[NL_OFFS] , forvar -> extra_flags )
		    == PARAMVAR )
#endif PC
	    ) ) {
	    standard();
	...
	if ( forvar->u.vals.value[ NL_FORV ] & FORVAR ) {
...
#	ifdef PC
		 ...
		 */
	    putRV( 0 , cbn , initnlp ->u.vals.value[ NL_OFFS ] ,
		    initnlp -> extra_flags , P2INT );
#	endif PC
#	ifdef OBJ
	    put(2, O_LV | cbn<<8+INDX, initnlp ->u.vals.value[ NL_OFFS ] );
#	endif OBJ
...
#	ifdef PC
		 ...
		 */
	    putRV( 0 , cbn , termnlp ->u.vals.value[ NL_OFFS ] ,
...
#	ifdef OBJ
		 ...
		 * compute and save the termination expression
		 */
	    put(2, O_LV | cbn<<8+INDX, termnlp ->u.vals.value[ NL_OFFS ] );
#	endif OBJ
...
#	ifdef OBJ
		 ...
		 * we can skip the loop altogether if !( init <= term )
		 */
	    put(2, O_RV4 | cbn<<8+INDX, initnlp ->u.vals.value[ NL_OFFS ] );
	    put(2, O_RV4 | cbn<<8+INDX, termnlp ->u.vals.value[ NL_OFFS ] );
...
#	endif OBJ
	     ...
	     */
	shadowed = TRUE;
	s_offset = forvar ->u.vals.value[ NL_OFFS ];
	s_flags = forvar -> nl_flags;
	forvar ->u.vals.value[ NL_OFFS ] = initnlp ->u.vals.value[ NL_OFFS ];
	forvar -> nl_flags = cbn;
	forvar ->u.vals.value[ NL_FORV ] = FORVAR;
#	ifdef PC
...
#	ifdef OBJ
	    ...
	    if (width(fortype) <= 2)
		    put(4, (arg[0] == T_FORU ? O_FOR1U : O_FOR1D) +
			    (width(fortype)>>1), (int)fortype->u.rngs.range[0],
			    (int)fortype->u.rngs.range[1], again);
	    else
		    put(4, (arg[0] == T_FORU ? O_FOR4U : O_FOR4D),
			    fortype->u.rngs.range[0], fortype->u.rngs.range[1], again);
		/*
		 * and here we are
		 */
	    patch( after );
...
byebye:
	noreach = 0;
	if (forvar != NIL) {
	    forvar ->u.vals.value[ NL_FORV ] = s_forv;
	}
	if ( shadowed ) {
	    forvar ->u.vals.value[ NL_OFFS ] = s_offset;
