	...
	int *r;
{
		...
		if (isnta(p, "bcsi")) {
			...
			if (cl == NIL)
				...
				if (hp21mx)
					w = 2;
#endif
				low = ((struct nl *)cl)->u.rngs.range[0];
				high = ((struct nl *)cl)->u.rngs.range[1];
			}
		}
	}
	/*
	 ...
	 */
	n = 0;
	for (cl = (int *)r[3]; cl != NIL; cl = (int *)cl[2]) {
		cs = (int *)cl[1];
		if (cs == NIL)
			continue;
		for (cs = (int *)cs[2]; cs != NIL; cs = (int *)cs[2])
			n++;
	}
	/*
	 * Allocate case table space
	 */
	ctab = (struct ct *)(i = malloc(n * sizeof *ctab));
	if (i == 0) {
