...
static char sccsid[] = "@(#)rval.c 1.12 6/1/81";
...
#			    ifdef OBJ
				...
				switch (w) {
					...
					put(3, O_RV | bn << 8+INDX,
						(int)p->u.vals.value[0], w);
				}
#			   endif OBJ
#			   ifdef PC
				if ( required == RREQ ) {
				    putRV( p -> symbol , bn , p ->u.vals.value[0] ,
					    p -> extra_flags , p2type( q ) );
				} else {
				    putLV( p -> symbol , bn , p ->u.vals.value[0] ,
					    p -> extra_flags , p2type( q ) );
				}
#			   endif PC
			   return (q);
...
#				    endif PC
			    ...
			    if (q == nl+T1CHAR) {
#				    ifdef OBJ
					put(2, O_CONC, (int)p->u.vals.value[0]);
#				    endif OBJ
#				    ifdef PC
					putleaf( P2ICON , p ->u.vals.value[0] , 0
						, P2CHAR , 0 );
#				    endif PC
				    return (q);
			    }
...
#ifndef DEBUG
#				    ifdef OBJ
					put(2, O_CON8, p->u.reals.real);
#				    endif OBJ
#				    ifdef PC
					putCON8( p ->u.reals.real );
#				    endif PC
#else
				    if (hp21mx) {
					    f = p->u.reals.real;
...
#					    endif PC
			    ...
			    case 4:
#				    ifdef OBJ
					put(2, O_CON4, p->u.rngs.range[0]);
#				    endif OBJ
#				    ifdef PC
					putleaf( P2ICON , p ->u.rngs.range[0] , 0
						, P2INT , 0 );
#				    endif PC
				    break;
			    case 2:
#				    ifdef OBJ
					put(2, O_CON2, (short)p->u.rngs.range[0]);
#				    endif OBJ
#				    ifdef PC
					    /*
					     * make short constants ints
					     */
					putleaf( P2ICON , (short) p ->u.rngs.range[0]
						, 0 , P2INT , 0 );
#				    endif PC
				    break;
			    case 1:
#				    ifdef OBJ
					put(2, O_CON1, p->u.vals.value[0]);
#				    endif OBJ
#				    ifdef PC
					    /*
...
#		ifdef PC
			...
			putLV( 0 , cbn , tempnlp ->u.vals.value[ NL_OFFS ] ,
	...
	switch (cn[0]) {
		...
		case T_VAR:
			...
			switch (classify(np->type)) {
				case TINT:
					con.crval = np->u.rngs.range[0];
					break;
				case TDOUBLE:
					con.crval = np->u.reals.real;
					break;
				case TBOOL:
				case TCHAR:
				case TSCAL:
					con.cival = np->u.vals.value[0];
					con.crval = con.cival;
					break;
				case TSTR:
					con.cpval = np->u.ptrs.ptr[0];
					break;
				default:
					con.ctype = NIL;
					return FALSE;
