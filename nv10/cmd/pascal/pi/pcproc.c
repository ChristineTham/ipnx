...
static	char sccsid[] = "@(#)pcproc.c 1.6 6/1/81";
	...
	case O_DISPOSE:
		...
		if (argv != NIL) {
			...
			for (; argv != NIL; argv = argv[2]) {
				...
				if (con.ctype == NIL)
					return;
				if (incompat(con.ctype, (ap->u.ptrs.ptr[NL_TAG])->type , NIL )) {
					cerror("Specified tag constant type clashed with variant case selector type");
					return;
				}
				for (ap = ap->u.ptrs.ptr[NL_VARNT]; ap != NIL; ap = ap->chain)
...
packunp:
		...
		j = al->u.rngs.range[1] - al->u.rngs.range[0] + 1;
		if (i < j) {
