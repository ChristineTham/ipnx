...
static char sccsid[] = "@(#)proc.c 1.9 6/4/81";
...
#define SKIP 16
	 ...
	 */
	argc = 0;
	for (al = (int *)argv; al != NIL; al = (int *)al[2])
		argc++;
	 ...
	 */
	op = p->u.vals.value[0] &~ NSTAND;
	if (opt('s') && (p->u.vals.value[0] & NSTAND)) {
		standard();
	...
	case O_WRITLN:
		...
		if (op == O_MESSAGE) {
			 ...
			 */
			put(1, O_MESSAGE);
		} else if (argv != NIL && (al = (int *)argv[1])[0] != T_WEXP) {
			/*
...
#						endif DEC11
		 ...
		 */
		switch (p->u.vals.value[0] &~ NSTAND) {
			case O_WRITEF:
				if (argc == 0)
		...
		for (; argv != NIL; argv = (int *)argv[2]) {
			...
			if (filetype != nl+T1CHAR) {
				...
				if (file != NIL)
					...
					put(2, PTR_RV, (int)input->u.vals.value[0]);
				put(1, O_FNIL);
	...
	case O_REWRITE:
		...
		if (argc == 2) {
			...
			al = (int *)argv[2];
			codeoff();
			al = (int *)stkrval(al[1], NOFLAGS , RREQ );
			codeon();
			if (al == NIL)
				return;
			if (classify(al) != TSTR) {
	...
	case O_DISPOSE:
		...
		if (argv != NIL) {
			...
			for (; argv != NIL; argv = (int *)argv[2]) {
				...
				if (con.ctype == NIL)
					return;
				if (incompat(con.ctype, ((struct nl *)ap->u.ptrs.ptr[NL_TAG])->type , NIL )) {
					cerror("Specified tag constant type clashed with variant case selector type");
					return;
				}
				for (ap = (struct nl *)ap->u.ptrs.ptr[NL_VARNT]; ap != NIL; ap = ap->chain)
					if (ap->u.rngs.range[0] == con.crval)
						break;
				if (ap == NIL) {
					error("No variant case label value equals specified constant value");
					return;
				}
				ap = (struct nl *)ap->u.ptrs.ptr[NL_VTOREC];
			}
		}
		put(2, op, width(ap));
		return;
...
packunp:
		...
		al = (int *)(struct nl *) stklval(puz, op == O_UNPACK ? NOFLAGS : MOD|NOUSE);
		codeon();
		if (ap == NIL)
			return;
		if (ap->class != ARRAY) {
		...
		if (((struct nl *)al)->type == NIL || ap->type == NIL)
			return;
		if (((struct nl *)al)->type != ap->type) {
		 ...
		 * j will be v-u+1
		 */
		i = ap->u.rngs.range[1] - ap->u.rngs.range[0] + 1;
		j = ((struct nl *)al)->u.rngs.range[1] - ((struct nl *)al)->u.rngs.range[0] + 1;
		if (i < j) {
