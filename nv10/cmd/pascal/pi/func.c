...
static char sccsid[] = "@(#)func.c 1.7 3/8/81";
	...
	switch (op) {
		...
		case O_PRED2:
			...
			if (isa(p1, "i")) {
				if (width(p1) <= 2) {
					op += O_PRED24 - O_PRED2;
					put(3, op, (int)p1->u.rngs.range[0],
						(int)p1->u.rngs.range[1]);
				} else {
					op++;
					put(3, op, p1->u.rngs.range[0], p1->u.rngs.range[1]);
				}
				return nl + T4INT;
			} else {
				put(3, op, (int)p1->u.rngs.range[0], (int)p1->u.rngs.range[1]);
				return p1;
			}
		case O_ODD2:
			if (isnta(p1, "i")) {
