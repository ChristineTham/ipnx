...
static char sccsid[] = "@(#)lval.c 1.8 6/15/81";
...
#			endif
		...
		case REF:
			 ...
			 */
			put(2, PTR_RV | bn << 8+INDX , (int)p->u.vals.value[0] );
	 ...
	 */
	if (c == NIL && (modflag&ASGN) && ( p->u.vals.value[NL_FORV] & FORVAR ) ) {
	...
	int *el;
{
	...
	for (i = 1; i <= d; i++) {
		...
		if (constsub = constval(el[1])) {
		    ap = con.ctype;
		    sub = con.crval;
		    if (sub < p->u.rngs.range[0] || sub > p->u.rngs.range[1]) {
			error("Subscript value of %ld is out of range", sub);
			return (0);
		    }
		    sub -= p->u.rngs.range[0];
		} else {
#		    ifdef PC
...
#		ifdef OBJ
		    ...
		    if (opt('t') == 0) {
			    ...
			    case 2:
			    case 1:
				    put(2, (width(ap) != 4 ? O_INX2P2 : O_INX4P2) | (w & ~1) << 7, ( short ) p->u.rngs.range[0]);
				    el = (int *)el[2];
				    continue;
			    }
		    }
		    put(4, width(ap) != 4 ? O_INX2 : O_INX4, w,
			(short)p->u.rngs.range[0], (short)(p->u.rngs.range[1]));
		    el = (int *)el[2];
		    continue;
#		endif OBJ
#		ifdef PC
