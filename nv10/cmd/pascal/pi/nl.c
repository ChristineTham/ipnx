...
initnl()
	...
	for ( ip = in_rclasses ; *ip != 0 ; ip ++ )
		...
		nl[*ip].type = np;
		np ->u.rngs.range[0] = *lp ++ ;
		np ->u.rngs.range[1] = *lp ++ ;
	...
	np ->u.rngs.range[0] = 0L;
	np ->u.rngs.range[1] = 127L;

	/*
	 *	alfa = array [ 1 .. 10 ] of char;
	 */
	np = defnl ( 0 , RANGE , nl+TINT , 0 );
	np ->u.rngs.range[0] = 1L;
	np ->u.rngs.range[1] = 10L;
...
#	endif PC
	...
	if (opt('s'))
		(nl + TBOOL)->chain = fp;
	hdefnl ( *cp++ , CONST , nl + T4INT , 0 ) ->u.rngs.range[0] = MININT;
	hdefnl ( *cp++ , CONST , nl + T4INT , 0 ) ->u.rngs.range[0] = MAXINT;
			...
			case CONST:
				switch (nloff(p->type)) {
					default:
						...
						break;
					case TDOUBLE:
						printf("\t%f", p->u.reals.real);
						break;
					case TINT:
					case T4INT:
con:
						printf("\t%ld", p->u.rngs.range[0]);
						break;
					case TSTR:
						printf("\t'%s'", p->u.ptrs.ptr[0]);
						break;
					}
				break;
			case VAR:
			...
			case SCAL:
			case RANGE:
				printf("\t%ld..%ld", p->u.rngs.range[0], p->u.rngs.range[1]);
				break;
			case RECORD:
				printf("\t%d(%d)", v, p->u.vals.value[NL_FLDSZ]);
				break;
			case FIELD:
				printf("\t%d", v);
				break;
			case STR:
				printf("\t|%d|", p->u.vals.value[0]);
				break;
			case FVAR:
			case FUNC:
			case PROC:
			case PROG:
				if (cbn == 0) {
					printf("\t<%o>", p->u.vals.value[0] & 0377);
#ifndef PI0
					if (p->u.vals.value[0] & NSTAND)
						printf("\tNSTAND");
#endif
					break;
				}
...
casedef:
		...
		switch (p->class) {
			case RECORD:
				if (p->u.ptrs.ptr[NL_VARNT])
					printf("\tVARNT=[%d]", nloff(p->u.ptrs.ptr[NL_VARNT]));
				if (p->u.ptrs.ptr[NL_TAG])
					printf(" TAG=[%d]", nloff(p->u.ptrs.ptr[NL_TAG]));
				break;
			case VARNT:
				printf("\tVTOREC=[%d]", nloff(p->u.ptrs.ptr[NL_VTOREC]));
				break;
		}
#		ifdef PC
		    if ( p -> extra_flags != 0 ) {
	...
	struct nl *p;
{
	...
	p1 = (int *)p;
	p = (struct nl *)(p2 = (int *)defnl(0, 0, 0, 0));
	i = (sizeof *p)/(sizeof (int));
	do
		*p2++ = *p1++;
	while (--i);
	...
	struct nl *np;
{
	...
	for (p = hp; p != NIL && (p->nl_flags & 037) == cbn; p = p->nl_next)
