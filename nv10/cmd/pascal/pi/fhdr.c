...
static char sccsid[] = "@(#)fhdr.c 1.3 6/1/81";
...
#			endif PC
			cp->chain = p;
			p->u.ptrs.ptr[NL_FVAR] = (int *)cp;
		}
		/*
		 * Enter the parameters
		 * and compute total size
		 */
	        p->u.vals.value[NL_OFFS] = params(p, r[3]);
		/*
		 ...
		 */
		if (p->class == FUNC)
		    ((struct nl *)p->u.ptrs.ptr[NL_FVAR])->u.vals.value[NL_LINENO] = r[1];
		else
		    p->u.vals.value[NL_LINENO] = r[1];
		cbn--;
	} else { 
		/*
		 * The wonderful
...
#		endif OBJ
	 ...
	 * the prog/proc/func.
	 */
	p->u.ptrs.entloc = getlab();
	if (monflg) {
		bodycnts[ cbn ] = getcnt();
		p->u.vals.value[ NL_CNTR ] = 0;
	}
#	ifdef OBJ
	    put(2, O_TRA4, (long)p->u.ptrs.entloc);
#	endif OBJ
#	ifdef PTREE
	    {
		pPointer	PF = tCopy( r );
...
{
	params(p, formal[3]);
	p ->u.vals.value[ NL_LINENO ] = formal[4];
	p ->u.ptrs.ptr[ NL_FCHAIN ] = (int *)p -> chain;
	p -> chain = NIL;
}
...
#	ifdef PC
	    ...
	    o = DPOFF2;
#	endif PC
	for (formalp = (int **)formalist; formalp != NIL; formalp = (int **)formalp[2]) {
		p = NIL;
		formal = formalp[1];
		if (formal == NIL)
			continue;
		...
		for (idlist = (int *)formal[1]; idlist != NIL; idlist = (int *)idlist[2]) {
			switch (formal[0]) {
			    default:
				    panic("funchdr2");
			    case T_PVAL:
...
#	ifdef OBJ
	    ...
	    for (dp = p->chain; dp != NIL; dp = dp->chain)
		    dp->u.vals.value[NL_OFFS] += -o + DPOFF2;
	    return (-o + DPOFF2);
#	endif OBJ
#	ifdef PC
