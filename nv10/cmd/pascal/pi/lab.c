...
static char sccsid[] = "@(#)lab.c 1.9 6/1/81";
...
#		ifdef OBJ
		     ...
		     */
		    putlab(l);
		    /* put(2, O_GOTO | cbn<<8+INDX, (long)p->u.vals.value[1]); */
		    put(2, O_GOTO | cbn<<8, (long)p->u.vals.value[1]);
#		endif OBJ
#		ifdef PC
		    /*
...
#	endif PC
	if (p->u.vals.value[NL_GOLEV] != NOTYET)
		if (p->u.vals.value[NL_GOLEV] < level) {
			recovered();
			error("Goto %s from line %d is into a structured statement", s, p->u.vals.value[NL_GOLINE]);
		}
	p->u.vals.value[NL_GOLEV] = level;
}
#endif
