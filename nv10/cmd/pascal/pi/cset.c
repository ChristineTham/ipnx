...
static char sccsid[] = "@(#)cset.c 1.6 3/20/81";
		...
		if (e[0] == T_RANG) {
			if ( csetp -> comptime && constval( e[2] ) ) {
				...
				if ( setofint ) {
				    ...
				    error("Range upper bound of %ld out of set bounds" , ((long)con.crval) );
				    csetp -> csettype = NIL;
				}
			    }
#endif CONSETS
			...
			if ( csetp -> comptime && constval( e[1] ) ) {
				...
				if ( setofint ) {
				    ...
				    error("Range lower bound of %ld out of set bounds" , ((long)con.crval) );
				    csetp -> csettype = NIL;
				}
			    }
#endif CONSETS
...
pairhang:
			...
			if ( csetp -> comptime && constval( e ) ) {
				...
				if ( setofint ) {
				    ...
				    error("Value of %ld out of set bounds" , ((long)con.crval) );
				    csetp -> csettype = NIL;
				}
			    }
#endif CONSETS
...
#	ifdef OBJ
	    if ( singp != NIL ) {
		for ( el = (int *)singp ; el[2] != NIL ; el = (int *)el[2] ) /* void */;
		el[2] = (int)pairp;
		r[2] = (int)singp;
	    } else {
		r[2] = (int)pairp;
...
#	endif PC
	...
	if ( csetp -> comptime ) {
		...
		*lp = 0;
	    ...
	    for ( el = (int *)r[2] ; el != NIL ; el = (int *)el[2] ) {
		e = (int *)el[1];
		if ( e[0] == T_RANG ) {
		    constval( e[1] );
		    lower = con.crval;
