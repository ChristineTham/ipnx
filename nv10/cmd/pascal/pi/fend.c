...
static char sccsid[] = "@(#)fend.c 1.9 6/8/81";
...
#	define HDRSZE (2 * sizeof(long) + sizeof(short) + sizeof(bool))
	var = put(2, ((lenstr(fp->symbol,0) + HDRSZE) << 8)
	     ...
	     */
	put(2, O_CASE4, cbn == 1 ? (long)0 : (long)(fp->u.vals.value[NL_OFFS]-DPOFF2));
	    /*
	...
	if ( fp -> class == PROG ) {
		...
		stabparam( p -> symbol , p2type( p -> type )
			    , p ->u.vals.value[ NL_OFFS ] , lwidth( p -> type ) );
	    }
	    if ( fp -> class == FUNC ) {
		    /*
		     *	stab the function variable
		     */
		p = fp ->u.ptrs.ptr[ NL_FVAR ];
...
#endif PC
	if ( monflg ) {
		if ( fp ->u.vals.value[ NL_CNTR ] != 0 ) {
			inccnt( fp ->u.vals.value [ NL_CNTR ] );
		}
		inccnt( bodycnts[ fp ->nl_flags & 037 ] );
	}
	if (fp->class == PROG) {
		/*
...
#			ifdef OBJ
			    ...
			    put(2, O_LVCON, i);
			    putstr(p->symbol, 0);
			    put(2, O_LV | bn<<8+INDX, (int)iop->u.vals.value[NL_OFFS]);
			    put(1, O_DEFNAME);
#			endif OBJ
#			ifdef PC
			    putleaf( P2ICON , 0 , 0
				    ...
				    , "_DEFNAME" );
			    putLV( p -> symbol , bn , iop ->u.vals.value[NL_OFFS] ,
...
#	endif PC
	...
	for (i = 0; i <= 077; i++) {
		for (p = disptab[i]; p != NIL && (p->nl_flags & 037) == b; p = p->nl_next) {
			/*
			...
			switch (p->class) {
				case BADUSE:
					...
					if (((struct udinfo *)p->chain)->ud_next == NIL)
						cp++;
					eholdnl();
					if (p->u.vals.value[NL_KINDS] & ISUNDEF)
...
#	ifdef PC
		 ...
		 */
	    if ( fp -> class == FUNC ) {
		struct nl	*fvar = fp ->u.ptrs.ptr[ NL_FVAR ];
		...
		switch ( classify( fvar -> type ) ) {
		    ...
		    case TDOUBLE:
		    case TPTR:
			putRV( fvar -> symbol , ( fvar ->nl_flags ) & 037 ,
				fvar ->u.vals.value[ NL_OFFS ] ,
				fvar -> extra_flags ,
				fvartype );
			break;
		    default:
			...
			putLV( fvar -> symbol , ( fvar ->nl_flags ) & 037 ,
				fvar ->u.vals.value[ NL_OFFS ] ,
				fvar -> extra_flags ,
				fvartype );
		...
		putleaf( P2ICON , 0 , 0 , ADDTYPE( P2FTN | P2INT , P2PTR ) ,
			"_FCALL" );
		putRV( 0 , cbn , fp ->u.vals.value[ NL_OFFS ] ,
...
#	endif PC
#ifdef DEBUG
	dumpnl(fp->u.ptrs.ptr[2], fp->symbol);
#endif
	/*
	 * Restore the
	 * (virtual) name list
