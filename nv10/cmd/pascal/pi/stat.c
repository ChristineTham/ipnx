...
static char sccsid[] = "@(#)stat.c 1.5 6/1/81";
	...
	int *r;
{
	...
	for (sl=(int *)r; sl != NIL; sl=(int *)sl[2])
		statement(sl[1]);
}
...
ungoto()
{
	...
	for (p = gotos[cbn]; p != NIL; p = p->chain)
		if ((p->nl_flags & NFORWD) != 0) {
			if (p->u.vals.value[NL_GOLEV] != NOTYET)
				if (p->u.vals.value[NL_GOLEV] > level)
					p->u.vals.value[NL_GOLEV] = level;
		} else
			if (p->u.vals.value[NL_GOLEV] != DEAD)
				if (p->u.vals.value[NL_GOLEV] > level)
					p->u.vals.value[NL_GOLEV] = DEAD;
}
	...
	int *s;
{
	...
	putline();
	swl = (int *)withlist;
	for (p = (int *)s[2]; p != NIL; p = (int *)p[2]) {
		tempnlp = tmpalloc(sizeof(int *), INT_TYP, REGOK);
#		ifdef OBJ
		    put(2, O_LV | cbn <<8+INDX, tempnlp ->u.vals.value[ NL_OFFS ] );
#		endif OBJ
#		ifdef PC
		    putRV( 0 , cbn , tempnlp ->u.vals.value[ NL_OFFS ] ,
...
#		endif PC
		...
		r = defnl(0, WITHPTR, r, tempnlp ->u.vals.value[ NL_OFFS ] );
#		ifdef PC
...
#			ifdef OBJ
			    put(2, O_LV | bn << 8+INDX, (int)p->u.vals.value[NL_OFFS]);
...
#			ifdef PC
				 ...
				 */
			    p = p ->u.ptrs.ptr[ NL_FVAR ];
			    putRV( p -> symbol , bn , p ->u.vals.value[ NL_OFFS ] ,
