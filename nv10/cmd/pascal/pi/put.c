...
static char sccsid[] = "@(#)put.c 1.14 3/26/81";
	...
	register struct nl *ap;
{
	...
	if (ap->class != TYPE)
		ap = ap->type;
	if (ap->u.vals.value[ NL_ELABEL ] != 0) {
		/* the list already exists */
		return( ap ->u.vals.value[ NL_ELABEL ] );
	}
#	ifdef OBJ
	    oldlc = (int)lc;
	    put(2, O_TRA, lc);
	    ap->u.vals.value[ NL_ELABEL ] = (int)lc;
#	endif OBJ
#	ifdef PC
	    putprintf( "	.data" , 0 );
	    putprintf( "	.align 1" , 0 );
	    ap ->u.vals.value[ NL_ELABEL ] = getlab();
	    putlab( ap ->u.vals.value[ NL_ELABEL ] );
#	endif PC
	/* number of scalars */
	next = ap->type;
	len = next->u.rngs.range[1]-next->u.rngs.range[0]+1;
#	ifdef OBJ
	    put(2, O_CASE2, len);
#	endif OBJ
#	ifdef PC
