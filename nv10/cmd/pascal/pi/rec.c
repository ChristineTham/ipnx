 ...
 * through ->chain.  Each VARNT has, in ->u.rngs.range[0] the value of
	...
	char first;
{
	...
	if (first)
		P0 = p;
#ifndef PI0
	p->u.vals.value[NL_FLDSZ] = p->u.vals.value[NL_OFFS] = off;
#endif
	if (r != NIL) {
		fields(p, r[2]);
		variants(p, r[3]);
	     ...
	     */
	p->u.vals.value[NL_OFFS] = roundup(p->u.vals.value[NL_OFFS], (long)align(p));
	P0 = P0was;
	return (p);
}
	...
	int *r;
{
	...
	for (fp = (int *)r; fp != NIL; fp = (int *)fp[2]) {
		tp = (int *)fp[1];
		if (tp == NIL)
			continue;
		jp = gtype(tp[3]);
	...
	register int *r;
{
	 ...
	 */
	p->u.ptrs.ptr[NL_TAG] = (int *)deffld(p, r[2], ct);
	for (vc = (int *)r[4]; vc != NIL; vc = (int *)vc[2]) {
		v = (int *)vc[1];
		if (v == NIL)
			continue;
		vr = (int *)tyrec1(v[3], p->u.vals.value[NL_FLDSZ], 0);
#ifndef PI0
		if (((struct nl *)vr)->u.vals.value[NL_OFFS] > p->u.vals.value[NL_OFFS])
			p->u.vals.value[NL_OFFS] = ((struct nl *)vr)->u.vals.value[NL_OFFS];
#endif
		line = v[1];
	...
	register struct nl *t;
{
	...
	if (reclook(P0, s) != NIL) {
	     ...
	     */
	fp = enter(defnl(s, FIELD, t, (int)roundup(p->u.vals.value[NL_OFFS],
			(long)align(t))));
#else
	...
	if (s != NIL) {
		...
		P0->chain = fp;
		     ...
		     */
		p ->u.vals.value[ NL_OFFS ] = fp ->u.vals.value[ NL_OFFS ] + width( t );
		p ->u.vals.value[ NL_FLDSZ ] = p ->u.vals.value[ NL_OFFS ];
#endif
		if (t != NIL) {
...
#		ifdef PC
		    stabfield( s , p2type( t ) , fp ->u.vals.value[ NL_OFFS ]
				, lwidth( t ) );
#		endif PC
	}
	return (fp);
	...
	register struct nl *ct;
{
	...
	if (ct != NIL)
		uniqv(p);
#endif
	av->chain = (struct nl *)p->u.ptrs.ptr[NL_VARNT];
	p->u.ptrs.ptr[NL_VARNT] = (int *)av;
	av->u.ptrs.ptr[NL_VTOREC] = (int *)vr;
	av->u.rngs.range[0] = con.crval;
	return (av);
}
	...
	struct nl *p;
{
	...
	for (vt = (struct nl *)p->u.ptrs.ptr[NL_VARNT]; vt != NIL; vt = vt->chain)
		if (vt->u.rngs.range[0] == con.crval) {
