...
static	char sccsid[] = "@(#)call.c 1.17 6/12/81";
	...
	int *argv, porf, psbn;
{
	...
	struct nl *flvalue();
	int *r;
	long	p_type_class = classify( p -> type );
	bool chk = TRUE;
#	ifdef PC
...
#	ifdef OBJ
		...
		put(2, PTR_RV | psbn << 8+INDX, (int)p->u.vals.value[NL_OFFS]);
	    }
	    if (porf == FUNC) {
		    /*
		     * Push some space
...
#	ifdef PC
		...
		tempdescrp = tmpalloc(sizeof( struct formalrtn *) , NIL ,
					REGOK );
		putRV( 0 , cbn , tempdescrp ->u.vals.value[ NL_OFFS ] ,
			tempdescrp -> extra_flags , P2PTR|P2STRTY );
		putRV( 0 , psbn , p ->u.vals.value[ NL_OFFS ] ,
		...
		case PROC:
			 ...
			 *	... p( ...
			 */
		    sextname( extname , p -> symbol , BLOCKNO(p ->nl_flags) );
...
#	ifdef OBJ
		...
		put(2, O_CALL | psbn << 8, (long)p->u.ptrs.entloc);
	    }
#	endif OBJ
#	ifdef PC
		/*
...
{
    if (p->class == FUNC)
	return (((struct nl *)(p->u.ptrs.ptr[NL_FVAR]))->u.vals.value[NL_LINENO]);
    return p->u.vals.value[NL_LINENO];
}
