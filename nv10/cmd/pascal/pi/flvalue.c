...
static char sccsid[] = "@(#)flvalue.c 1.11 6/8/81";
...
#		    ifdef OBJ
			put(2, PTR_RV | bn << 8+INDX, (int)p->u.vals.value[NL_OFFS]);
#		    endif OBJ
#		    ifdef PC
...
#		    endif PC
	    ...
	    case PROC:
			 ...
			 */
		    tempnlp = tmpalloc( sizeof ( struct formrtnhead )
				+ 2 * bn * sizeof(struct dispsave) ,
				nl+TSTR, NOREG);
#		    ifdef OBJ
			put(2 , O_LV | cbn << 8 + INDX ,
				(int)tempnlp ->u.vals.value[ NL_OFFS ] );
			put(2, O_FSAV | bn << 8, (long)p->u.ptrs.entloc);
#		    endif OBJ
#		    ifdef PC
			putleaf( P2ICON , 0 , 0 ,
