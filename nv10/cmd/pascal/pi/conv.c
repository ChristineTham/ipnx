...
static char sccsid[] = "@(#)conv.c 1.2 3/8/81";
...
#	ifdef OBJ
	    ...
	    switch (rp->class) {
	    case RANGE:
		    if (rp->u.rngs.range[0] != 0) {
#    		    ifndef DEBUG
			    if (wrp <= 2)
				    put(3, O_RANG2+op, ( short ) rp->u.rngs.range[0],
						     ( short ) rp->u.rngs.range[1]);
			    else if (rp != nl+T4INT)
				    put(3, O_RANG4+op, rp->u.rngs.range[0], rp->u.rngs.range[1] );
#    		    else
			    if (!hp21mx) {
				    if (wrp <= 2)
					    put(3, O_RANG2+op,( short ) rp->u.rngs.range[0],
					    ...
					    put(3, O_RANG4+op,rp->u.rngs.range[0],
							     rp->u.rngs.range[1]);
			    } else
...
#    		    endif
	    ...
	    case SCAL:
		    if (wrp <= 2)
			    put(2, O_RSNG2+op, ( short ) rp->u.rngs.range[1]);
		    else
			    put( 2 , O_RSNG4+op, rp->u.rngs.range[1]);
		    break;
	    default:
		    panic("rangechk");
	    }
	...
	switch ( p -> class ) {
		...
		if ( p != nl + T4INT ) {
		    putleaf( P2ICON , 0 , 0 , ADDTYPE( P2FTN | P2INT , P2PTR )
			    , p ->u.rngs.range[0] != 0 ? name1 : name2 );
		}
		break;
	    case SCAL:
		    /*
...
postcheck( p )
	...
	switch ( p -> class ) {
	    case RANGE:
		if ( p != nl + T4INT ) {
		    if (p ->u.rngs.range[0] != 0 ) {
			putleaf( P2ICON , p ->u.rngs.range[0] , 0 , P2INT , 0 );
			putop( P2LISTOP , P2INT );
		    }
		    putleaf( P2ICON , p ->u.rngs.range[1] , 0 , P2INT , 0 );
		    putop( P2LISTOP , P2INT );
		    putop( P2CALL , P2INT );
		}
		break;
