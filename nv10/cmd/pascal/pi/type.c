	...
	register int *r;
{
	...
	switch (r[0]) {
		...
		case T_ID:
			...
			break;
		case T_TYSCAL:
			np = (struct nl *)tyscal(r);
			break;
		case T_TYRANG:
			np = (struct nl *)tyrang(r);
			break;
		case T_TYPTR:
			np = defnl(0, PTR, 0, 0 );
			np ->u.ptrs.ptr[0] = (int *)r[2];
	...
	register int *r;
{
	...
	lp->u.rngs.range[0] = con.crval;
	lp->u.rngs.range[1] = high;
	return (lp);
}
	...
	int *r;
{
	...
	ltp = np;
	i = 0;
	for (tl = (int *)r[2]; tl != NIL; tl = (int *)tl[2]) {
		tp = gtype(tl[1]);
		if (tp == NIL) {
			np = NIL;
			continue;
...
#ifndef PC
		if (tp->class == RANGE && bytes(tp->u.rngs.range[0], tp->u.rngs.range[1]) > 2) {
#ifndef PI1
...
foredecl()
{
	...
	for (p = forechain; p != NIL; p = p->nl_next) {
		if (p->class == PTR && p ->u.ptrs.ptr[0] != 0)
		{
			p->type = gtype(p ->u.ptrs.ptr[0]);
#ifndef PI1
...
#			ifdef PTREE
			{
				...
				pPointer	PtrTo = tCopy( p ->u.ptrs.ptr[0] );
