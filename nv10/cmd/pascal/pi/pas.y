...
prog_hedr:
	YPROG YID '(' id_list ')' ';' 
		= $$ = (int *)funcbody(funchdr(tree5(T_PROG, lineof($1), $2, fixlist($4), NIL)));
		|
	YPROG error
		= {
			 ...
			 */
			$$ = (int *)funcbody(funchdr(tree5(T_PROG, lineof($1), NIL, NIL, NIL)));
		  }
		;
block:
...
phead:
	porf YID params ftype ';'
		= $$ = (int *)funchdr(tree5($1, lineof($5), $2, $3, $4));
		;
porf:
	YPROCEDURE
		= $$ = (int *)T_PDEC;
...
stat_lsth:
	stat_list ';'
		= if ((q = $1) != NIL && (p = (int *)q[1]) != NIL && p[0] == T_IF) {
			if (yychar < 0)
				yychar = yylex();
...
qual_var:
	array_id '[' expr_list ']'
		...
		|
	qual_var '[' expr_list ']'
		= $1[3] = (int)addlist($1[3], tree2(T_ARY, fixlist($3)));
		|
	record_id '.' field_id
		= $$ = setupvar($1, setupfield($3, NIL));
		|
	qual_var '.' field_id
		= $1[3] = (int)addlist($1[3], setupfield($3, NIL));
		|
	ptr_id '^'
		= $$ = setupvar($1, tree1(T_PTR));
		|
	qual_var '^'
		= $1[3] = (int)addlist($1[3], tree1(T_PTR));
		;

/*
