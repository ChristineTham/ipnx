...
static char sccsid[] = "@(#)yyid.c 1.3 3/8/81";
	...
	int line, kindmask;
{
	...
	p->u.vals.value[NL_KINDS] |= kindmask;
	yybadref(p, line);
	return (oldp);
}
	...
	int kind;
{
	...
	if (p->class == BADUSE) {
		if (kind == VAR)
			return (p->u.vals.value[0] & varkinds);
		return (p->u.vals.value[0] & (1 << kind));
	}
	if (yyidok1(p, kind))
		return (1);
	if (yyidhave != NIL)
