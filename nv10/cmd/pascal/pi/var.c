...
#endif
		case SCAL:
			return (bytes(p->u.rngs.range[0], p->u.rngs.range[1]));
		case SET:
			setran(p->type);
...
alignit:
	switch ( p -> class ) {
	    ...
	    case RANGE:
		    ...
		    /* else, fall through */
	    case SCAL:
		    switch ( bytes( p ->u.rngs.range[0] , p ->u.rngs.range[1] ) ) {
			case 4:
			    return A_LONG;
			case 2:
			    return A_SHORT;
	...
	int n;
{
	...
	while (p != NIL) {
		if (p->class != RANGE && p->class != SCAL)
			panic("aryran");
		d = p->u.rngs.range[1] - p->u.rngs.range[0] + 1;
		s *= d;
		p = p->chain;
	}
	return (s);
