	...
	struct nl *fp;
{
	     ...
	     */
	if ( monflg ) {
	    fp ->u.vals.value[ NL_CNTR ] = bodycnts[ cbn ];
	}
	return (fp);
}
...
#			endif PC
	...
	if (fp->class == FUNC) {
		 ...
		 */
		enter(fp->u.ptrs.ptr[NL_FVAR]);
#		ifdef PC
		    q = fp ->u.ptrs.ptr[ NL_FVAR ];
		    if (q -> type != NIL ) {
			functemp = tmpalloc(
					leven(
					    roundup(
						...
						(long)align(q -> type))),
					q -> type, NOREG);
			if ( q ->u.ptrs.ptr[NL_OFFS] != functemp->u.vals.value[NL_OFFS] )
			    panic("func var");
		    }
