static	char *sccsid = "@(#)yycomm.c	1.1 (Berkeley) 3/2/81";
	...
	char cmdelim;
{
	...
	cp = (struct comment *)tree5(NIL, cmdelim, NIL, cmjust, yyseqid);
	cmeof = 0;
	do {
		kp = (struct commline *)getcmline(cmdelim);
		if (cp->cml == NIL) {
			kp->cml = kp;
			kp->cmcol = col;
		} else {
	...
	char cmdelim;
{
	...
	c = readch();
	kp = (struct commline *)tree3(NIL, yycol, NIL);
	tp = token;
	lastc = 0;
	for (;;) {
		switch (c) {
